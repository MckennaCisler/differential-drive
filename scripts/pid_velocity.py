...
######################################################
    ...
    #####################################################
        ...
        self.Kf = rospy.get_param('~Kf',125) # PIDF feed-forward coefficent
        ...
        rospy.logdebug("%s got Kp:%0.3f Ki:%0.3f Kd:%0.3f Kf:%0.3f tpm:%0.3f" % (self.nodename, self.Kp, self.Ki, self.Kd, self.Kf, self.ticks_per_meter))
    ...
    #####################################################
        ...
        # PIDF control with feed-forward; add a component proprotional to the target velocity
        # command that sets the theoretical power required and let the PID components correct
        self.motor = (self.Kf * self.target) + (self.Kp * self.error) + (self.Ki * self.integral) + (self.Kd * self.derivative)
